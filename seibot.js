...
export const seiFunctionBot = async (newMnemonics) => {
    ...
    const fee = calculateFee(100000, "0.01usei");
    ...
    for (let j=0;j<=2000;j++) {
        ...
        for (let i = 0; i < newMnemonics.length; i += 2) {
            const transaction = async () => {
                try {
                    ...
                    console.error(`Error occurred while processing transactions: ${error.message}`);
                }
        ...
        await Promise.allSettled(transactions).catch(err => console.log(err));
